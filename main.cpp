...
//*词法分析表
#define K_main 1       // main
#define K_INT 2        // int
#define K_num 3        //数字
#define K_CHAR 4       //字符
#define K_double 5     // double
#define K_if 6         // if
#define K_else 7       // else
#define K_IDENTIFIER 9 //标识符y
#define K_dengyu 21    // 等号
#define K_jiahao 22    //加号
#define K_jianhao 23   //减号
#define K_chenghao 24  //乘号
#define K_chuhao 25    //除号
#define K_rightk 26    //左括号
#define K_leftk 27     //右括号
#define K_leftd 28     //左大括号
#define K_rightd 29    //右大括号
#define K_doudian 30   // 分号
#define k_dayu 31      //大于号
#define K_xiaoyu 32    //小于号
using namespace std;
...
//保存到目标代码
void add_target_code(string dsf, string op, string dst, string dsc, string mark,
                     string step) {
    Target tmp;
    tmp.dsf = dsf;
    tmp.op = op;
...
void lexical_analysis(string source, vector<IDwords> &AnalysisResults) {
    char ch;
    ifstream rfile = ifstream(source.c_str());
    if (!rfile.is_open()) {
        cout << "无法打开源文件" << endl;
        exit(-1);
    }
    ...
    for (auto &it : AnalysisResults) {
        ...
        // compare,当==0时，说明两个字符串相等，不相等则返回-1
        if (it.word.compare(s1) == 0)
            cout << "符号编码："
                 << "    " << 1 << "    " << it.word << endl;
        else if (it.word == s2)
            cout << "符号编码："
    ...
    for (j = 1; j < str.size() - 1;) {
        if (str[j] == '%') {
            if (i != j) {
                ...
                if (vab == 91)
                    vab = '0';
                add_target_code("\'" + str.substr(i, j - i) + "$\'", "p", " ", " ",
                                "tmp" + char_to_str(vab), " ");
    ...
    typedef struct Target
  {
    ...
    */
    ...
    for (it = target_code.begin(); it != target_code.end(); it++) {
        ...
        if (it->op == "=") {
            printf("%s    %4s    %4s\n", it->op.c_str(), it->dst.c_str(),
                   it->dsf.c_str());
        } else if (it->op == "if") {
            printf("%s    %4s    %4s    %4s\n", it->mark.c_str(), it->dst.c_str(),
                   it->dsc.c_str(), it->step.c_str());
        } else if (it->op == "jmp") {
            printf("%s    %4s\n", it->op.c_str(), it->step.c_str());
        } else if (it->op == "pstep") {
            printf("%s    %4s\n", it->op.c_str(), it->step.c_str());
        } else if (it->op == "+" || it->op == "-") {
            printf("%s    %4s    %4s    %4s\n", it->op.c_str(), it->dst.c_str(),
                   it->dsc.c_str(), it->dsf.c_str());
        } else {
            cout << it->op << "    " << it->dsc << "    " << it->dst << "    "
                 << it->dsf << "    " << it->mark << "    " << it->step << endl;
        }
        //加减法转化
        if (it->op == "+" || it->op == "-")
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 1) {
        cout << "+--------------------------------------------------+" << endl
             ...
             << "|                编译原理课程设计                  |" << endl
             << "|--------------------------------------------------|" << endl
             << "|班级：         计算机科学与技术17-01              |" << endl
             << "|学号：541707010103    541707010114   541723030123 |" << endl
             << "|姓名：   陈留阳           黄悦          张飞燕    |" << endl
             << "+--------------------------------------------------+" << endl
             << "\n请输入源文件：";
        cin >> source;
        ...
        //生成汇编文件
        // asmfile(source)用来生成.asm文件，生成后与源文件路径相同
